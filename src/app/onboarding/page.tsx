...
const TIMELINE_DAYS: Record<string,number> = {'2 weeks':14,'1 month':30,'2 months':60,'3 months':90,'6 months':180,'1 year':365,'2+ years':730}
...
export default function OnboardingPage() {
  ...
  useEffect(()=>{
    const load = async () => {
      ...
      if ((count||0) >= (isPro?5:2)) {
        toast.error(`Goal limit reached. ${isPro?'':'Upgrade to Pro for more.'}`)
        router.push('/dashboard/goals')
      }
      // Pre-fill appearance from profile
      if (prof?.gender) upd('gender', prof.gender)
      if (prof?.age_range) upd('ageRange', prof.age_range)
      if (prof?.ethnicity) upd('ethnicity', prof.ethnicity)
    }
    load()
  },[])
  ...
  const validate = () => {
    ...
    if (step===4 && (!data.gender||!data.ageRange)) return 'Please answer the appearance questions'
  ...
  const generate = async () => {
    ...
    try {
      ...
      toast.error('Generation issue — using defaults')
      const days = TIMELINE_DAYS[data.timeline]||90
      setResult({
        displayTitle: data.goal.split(' ').slice(0,5).join(' '),
        artTitle:'The Vision', artDescription:`A ${data.gender?.toLowerCase()||'person'} actively achieving: ${data.goal}.`,
        visionArtPrompt:'', affirmation:`I am working toward ${data.goal} every day.`,
        milestones:[
          {label:`Day ${Math.round(days*.33)}`,goal:`Foundation: first major step toward ${data.goal}`},
          {label:`Day ${Math.round(days*.66)}`,goal:`Momentum: halfway progress with measurable results`},
          {label:`Day ${days}`,goal:`Achievement: ${data.goal} completed`},
        ],
        coachOpening:`You just took the most important step — getting specific. Let's build on that.`,
        todayAction:`Write down the single most important action you can take today toward your goal.`
      })
    }
    setGenerating(false)
  }
  ...
  const activate = async () => {
    ...
    const {error} = await supabase.from('goals').insert({
      user_id:user.id, title:data.goal, display_title:result.displayTitle,
      category:data.category, timeline:data.timeline, why:data.why,
      obstacles:data.obstacles, aesthetic:data.aesthetic,
      art_title:result.artTitle, art_description:result.artDescription,
      affirmation:result.affirmation,
      milestone_30:result.milestones?.[0]?.goal,
      milestone_60:result.milestones?.[1]?.goal,
      milestone_90:result.milestones?.[2]?.goal,
      coach_opening:result.coachOpening, today_action:result.todayAction,
      vision_art_prompt:result.visionArtPrompt,
      coach_style:data.coachStyle, motivator:data.motivator,
      best_time:data.bestTime, success_looks:data.successLooks,
      user_gender:data.gender, user_age_range:data.ageRange, user_ethnicity:data.ethnicity,
      is_active:true, vision_board_last_generated:new Date().toISOString(),
    })
        ...
        <div className="p-10">
          ...
          {step===0 && (
            <div className="fade-up">
              ...
              <p className="text-[14px] text-[#666] mb-6">Be specific. Vague goals stay wishes.</p>
              <div className="mb-5">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {CATS.map(c=><button key={c} onClick={()=>upd('category',c)} className={`px-3.5 py-2 rounded-full text-[13px] border transition-all ${data.category===c?'bg-[#111] text-white border-[#111]':'bg-white text-[#111] border-[#e8e8e8] hover:border-[#d0d0d0]'}`}>{c}</button>)}
                </div>
              </div>
            </div>
          )}
          ...
          {step===1 && (
            <div className="fade-up">
              ...
              <p className="text-[14px] text-[#666] mb-6">Your milestones will be calibrated exactly to this timeframe.</p>
              <div className="flex flex-wrap gap-2 mb-5">
                {TLS.map(t=><button key={t} onClick={()=>upd('timeline',t)} className={`px-4 py-2.5 rounded-full text-[13px] border transition-all ${data.timeline===t?'bg-[#b8922a] text-white border-[#b8922a]':'bg-white text-[#111] border-[#e8e8e8] hover:border-[#d0d0d0]'}`}>{t}</button>)}
              </div>
              {data.timeline && <div className="p-4 bg-[#faf3e0] rounded-xl border border-[#b8922a]/20 text-[13px] text-[#666]">Milestones set for <strong className="text-[#b8922a]">{data.timeline}</strong> — {TIMELINE_DAYS[data.timeline]} days</div>}
            </div>
          )}
          ...
          {step===2 && (
            <div className="fade-up">
              ...
              <p className="text-[14px] text-[#666] mb-6">This powers your coaching. The more honest, the better.</p>
              <div className="mb-4">
                <label className="block text-[12px] font-medium text-[#666] mb-2">Why does this truly matter?</label>
              ...
              <div>
                ...
                <textarea className="w-full px-3.5 py-3 border border-[#e8e8e8] rounded-xl text-[14px] outline-none focus:border-[#111] transition-colors resize-none" rows={2} value={data.obstacles} onChange={e=>upd('obstacles',e.target.value)} placeholder="Knowing your patterns helps your coach"/>
              </div>
            </div>
          )}
          ...
          {step===3 && (
            <div className="fade-up">
              ...
              <p className="text-[14px] text-[#666] mb-6">Your AI coach adapts entirely to how you work best.</p>
              <div className="mb-5">
                <label className="block text-[12px] font-medium text-[#666] mb-2">What motivates you most?</label>
                <div className="flex flex-wrap gap-2">{MOTIVATORS.map(m=><button key={m} onClick={()=>upd('motivator',m)} className={`px-3 py-1.5 rounded-full text-[12px] border transition-all ${data.motivator===m?'bg-[#111] text-white border-[#111]':'bg-white border-[#e8e8e8] hover:border-[#d0d0d0]'}`}>{m}</button>)}</div>
              </div>
              <div className="mb-5">
                <label className="block text-[12px] font-medium text-[#666] mb-2">When are you most productive?</label>
                <div className="flex flex-wrap gap-2">{TIMES.map(t=><button key={t} onClick={()=>upd('bestTime',t)} className={`px-3 py-1.5 rounded-full text-[12px] border transition-all ${data.bestTime===t?'bg-[#111] text-white border-[#111]':'bg-white border-[#e8e8e8] hover:border-[#d0d0d0]'}`}>{t}</button>)}</div>
              </div>
              <div>
                <label className="block text-[12px] font-medium text-[#666] mb-2">How do you prefer to be coached?</label>
                <div className="flex flex-wrap gap-2">{STYLES.map(s=><button key={s} onClick={()=>upd('coachStyle',s)} className={`px-3 py-1.5 rounded-full text-[12px] border transition-all ${data.coachStyle===s?'bg-[#111] text-white border-[#111]':'bg-white border-[#e8e8e8] hover:border-[#d0d0d0]'}`}>{s}</button>)}</div>
              </div>
            </div>
          )}
          ...
          {step===4 && (
            <div className="fade-up">
              ...
              <p className="text-[14px] text-[#666] mb-2">We generate art showing <em>you</em> achieving your goal — not a generic person.</p>
              <p className="text-[11px] text-[#999] mb-6">Used only for art generation. Never shared.</p>
              <div className="mb-5">
                <label className="block text-[12px] font-medium text-[#666] mb-2">Gender</label>
                <div className="flex flex-wrap gap-2">{GENDERS.map(g=><button key={g} onClick={()=>upd('gender',g)} className={`px-4 py-2 rounded-full text-[13px] border transition-all ${data.gender===g?'bg-[#111] text-white border-[#111]':'bg-white border-[#e8e8e8] hover:border-[#d0d0d0]'}`}>{g}</button>)}</div>
              </div>
              <div className="mb-5">
                <label className="block text-[12px] font-medium text-[#666] mb-2">Age range</label>
                <div className="flex flex-wrap gap-2">{AGES.map(a=><button key={a} onClick={()=>upd('ageRange',a)} className={`px-4 py-2 rounded-full text-[13px] border transition-all ${data.ageRange===a?'bg-[#111] text-white border-[#111]':'bg-white border-[#e8e8e8] hover:border-[#d0d0d0]'}`}>{a}</button>)}</div>
              </div>
              <div>
                ...
                <div className="flex flex-wrap gap-2">{ETHNICITIES.map(e=><button key={e} onClick={()=>upd('ethnicity',e)} className={`px-3 py-1.5 rounded-full text-[12px] border transition-all ${data.ethnicity===e?'bg-[#111] text-white border-[#111]':'bg-white border-[#e8e8e8] hover:border-[#d0d0d0]'}`}>{e}</button>)}</div>
              </div>
            </div>
          )}
          ...
          {step===5 && (
            <div className="fade-up">
              ...
              <p className="text-[14px] text-[#666] mb-6">Your art will show you achieving your goal in this style.</p>
            ...
            <div className="fade-up">
              {generating ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-[14px] text-[#666]">Creating personalized art and coaching just for you</p>
                </div>
              ) : result && (
                <>
                  ...
                  {result.displayTitle && (
                    <div className="bg-[#faf3e0] border border-[#b8922a]/20 rounded-xl p-4 mb-4">
                      <p className="text-[10px] font-medium tracking-[.1em] uppercase text-[#b8922a] mb-1">Your journey title</p>
                      <p className="font-serif text-[20px]">{result.displayTitle}</p>
                    </div>
                  )}
                  ...
                  <div className="bg-[#111] rounded-xl p-4 mb-5">
                    ...
                    <div className="bg-white/[0.06] rounded-lg p-3 text-[12px] text-white/55"><strong className="text-white/80">Today: </strong>{result.todayAction}</div>
                  </div>
          ...
          {step<6 && (
            <div className="flex justify-between items-center mt-7 pt-5 border-t border-[#e8e8e8]">
              ...
              <button onClick={next} className="px-5 py-2.5 bg-[#111] text-white rounded-lg text-[13px] font-medium hover:bg-[#2a2a2a] transition-colors">{step===5?'Create my manifest ✦':'Continue →'}</button>
            </div>
          )}
        </div>
      </div>
