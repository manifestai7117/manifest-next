...
function getCollageImgs(goal: any, regenCount: number): string[] {
  ...
  let key = 'default'
  if (t.includes('cod') || t.includes('program') || t.includes('develop') || t.includes('software')) key = 'coding'
...
export default function ArtPage() {
  ...
  const generate = async () => {
    ...
    try {
      ...
      const updated = { ...selectedGoal, art_image_url: data.imageUrl, vision_board_regenerations: newCount }
  ...
  const downloadCollage = async () => {
    ...
    if (!isPro) { toast.error('Download available for Pro users only'); return }
    // Open all images in new tab as download (simple approach)
    toast.success('Preparing download...')
    const imgs = getCollageImgs(selectedGoal, regenCount)
    imgs.forEach((id, i) => {
      setTimeout(() => {
        const a = document.createElement('a')
        a.href = `https://images.unsplash.com/${id}?w=1200&h=1200&fit=crop&crop=center&fm=jpg&q=90`
        a.download = `vision-board-${i + 1}.jpg`
        a.target = '_blank'
  ...
  return (
    <div className="fade-up max-w-[900px]">
      <div className="flex items-start justify-between mb-4 flex-wrap gap-3">
        <div>
          <h1 className="font-serif text-[32px] mb-1">Vision Board</h1>
          <p className="text-[14px] text-[#666]">Your visual world — curated for <em>{selectedGoal.title}</em></p>
        </div>
        <div className="flex gap-2 flex-wrap">
          ...
          <button onClick={generate} disabled={generating}
            ...
            {generating ? (
              ...
              {selectedGoal.art_image_url ? 'New image' : 'Generate AI art'}</>
            )}
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* LEFT — Collage grid */}
        <div ref={collageRef}>
          <div className="grid grid-cols-3 gap-1.5 rounded-2xl overflow-hidden mb-1.5" style={{ display: 'grid', gridTemplateRows: 'auto auto' }}>
            {/* Large top-left */}
            <div className="overflow-hidden col-span-2 row-span-2" style={{ aspectRatio: '1/1' }}>
            ...
            <div className="overflow-hidden" style={{ aspectRatio: '1/1' }}>
              <img src={`https://images.unsplash.com/${imgs[1]}?w=400&h=400&fit=crop&crop=center&q=90`}
                alt="" className="w-full h-full object-cover hover:scale-105 transition-transform duration-700"/>
            </div>
            <div className="overflow-hidden" style={{ aspectRatio: '1/1' }}>
              <img src={`https://images.unsplash.com/${imgs[2]}?w=400&h=400&fit=crop&crop=center&q=90`}
                alt="" className="w-full h-full object-cover hover:scale-105 transition-transform duration-700"/>
            </div>
            {/* Bottom row 3 equal */}
            <div className="overflow-hidden" style={{ aspectRatio: '1/1' }}>
              <img src={`https://images.unsplash.com/${imgs[3]}?w=400&h=400&fit=crop&crop=center&q=90`}
                alt="" className="w-full h-full object-cover hover:scale-105 transition-transform duration-700"/>
            </div>
            <div className="overflow-hidden" style={{ aspectRatio: '1/1' }}>
              <img src={`https://images.unsplash.com/${imgs[4]}?w=400&h=400&fit=crop&crop=center&q=90`}
                alt="" className="w-full h-full object-cover hover:scale-105 transition-transform duration-700"/>
            </div>
            <div className="overflow-hidden" style={{ aspectRatio: '1/1' }}>
              <img src={`https://images.unsplash.com/${imgs[5]}?w=400&h=400&fit=crop&crop=center&q=90`}
                alt="" className="w-full h-full object-cover hover:scale-105 transition-transform duration-700"/>
            </div>
          </div>
          ...
          {selectedGoal.art_image_url && (
            <div className="rounded-2xl overflow-hidden aspect-[3/2] relative mb-1.5">
              <img src={selectedGoal.art_image_url} alt="AI generated vision" className="w-full h-full object-cover"/>
              <div className="absolute top-2 right-2 bg-black/50 text-white text-[10px] px-2 py-1 rounded-full">AI Generated</div>
            </div>
          )}
        ...
        <div className="space-y-4">
          ...
          {selectedGoal.vision_board_regenerations > 0 && (
            <p className="text-[11px] text-[#999]">Board regenerated {selectedGoal.vision_board_regenerations} time{selectedGoal.vision_board_regenerations !== 1 ? 's' : ''} · each time shows new images</p>
          )}
