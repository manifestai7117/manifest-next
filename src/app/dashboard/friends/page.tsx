...
export default function FriendsPage() {
  ...
  const [blockModal, setBlockModal] = useState<any>(null) // user object to confirm block
  ...
  const load = async () => {
    ...
    const [
      ...
      supabase.from('blocked_users')
        .select('blocked_id, profile:profiles!blocked_users_blocked_id_fkey(id, full_name, avatar_url, plan)')
    ...
    setBlockedUsers((blocked || []).map((b: any) => b.profile).filter(Boolean))
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`dm-${[user.id, activeChat.id].sort().join('-')}`)
  ...
  const confirmBlock = async () => {
    ...
    setBlockedUsers(prev => [...prev, blockModal])
  ...
  const unblock = async (blockedId: string) => {
    ...
    setBlockedUsers(prev => prev.filter((u: any) => u.id !== blockedId))
  ...
  const clearDM = (friendId: string) => {
    ...
    localStorage.setItem('dm_cleared_at', JSON.stringify(updated))
    setMessages([])
  ...
  const sendDM = async () => {
    ...
    const { data: msg } = await supabase.from('direct_messages').insert({
      sender_id: user.id, recipient_id: activeChat.id,
      content: content || null, media_url: dmMediaUrl || null, media_type: dmMediaType || null,
    }).select().single()
    ...
    <div className="fade-up max-w-[900px]">
          ...
          <div className="bg-white rounded-2xl max-w-[380px] w-full p-6 shadow-2xl">
            <div className="text-center mb-5">
              ...
              <p className="text-[13px] text-[#666] leading-[1.6]">
                They won't be able to see your profile, posts, or send you messages. You can unblock them anytime.
              </p>
            </div>
            <div className="flex gap-3">
              <button onClick={() => setBlockModal(null)} className="flex-1 py-2.5 border border-[#e8e8e8] rounded-xl text-[13px] hover:bg-[#f8f7f5]">Cancel</button>
      ...
      {unblockTarget && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-6">
          <div className="bg-white rounded-2xl max-w-[380px] w-full p-6 shadow-2xl">
            <div className="text-center mb-5">
              <h3 className="font-serif text-[20px] mb-2">Unblock {unblockTarget.full_name}?</h3>
              <p className="text-[13px] text-[#666]">They'll be able to see your profile and send requests again.</p>
            </div>
            <div className="flex gap-3">
              <button onClick={() => setUnblockTarget(null)} className="flex-1 py-2.5 border border-[#e8e8e8] rounded-xl text-[13px]">Cancel</button>
              <button onClick={() => unblock(unblockTarget.id)} className="flex-1 py-2.5 bg-[#111] text-white rounded-xl text-[13px] font-medium">Unblock</button>
            </div>
          </div>
        </div>
      )}
        ...
        {([
          ...
          <button key={id} onClick={() => setTab(id as any)}
            ...
            {id === 'requests' && requests.length > 0 && (
              <span className="w-4 h-4 bg-red-500 text-white text-[9px] font-bold rounded-full inline-flex items-center justify-center">{requests.length}</span>
            )}
            {id === 'blocked' && blockedUsers.length > 0 && (
              <span className="w-4 h-4 bg-[#999] text-white text-[9px] font-bold rounded-full inline-flex items-center justify-center">{blockedUsers.length}</span>
            )}
          </button>
        ))}
      </div>
              ...
              : users.map(u => (
                <div key={u.id} className="bg-white border border-[#e8e8e8] rounded-2xl p-4 flex items-center gap-3 hover:border-[#d0d0d0] transition-all">
                  ...
                  <div className="flex gap-2 items-center">
                    {u.isPending
                      ? <button onClick={() => cancelRequest(u.id)} className="text-[11px] font-medium text-[#b8922a] bg-[#faf3e0] px-3 py-1.5 rounded-full hover:bg-[#f5e8c8] transition-colors">Pending ✕</button>
                    ...
                    <button onClick={() => setBlockModal(u)} className="w-7 h-7 flex items-center justify-center text-[#ccc] hover:text-red-400 rounded-full transition-colors" title="Block">
          ...
          {!activeChat
            ...
            : <div className="bg-white border border-[#e8e8e8] rounded-2xl overflow-hidden flex flex-col" style={{ height: 520 }}>
                ...
                <div className="px-5 py-3.5 border-b border-[#e8e8e8] flex items-center gap-3 flex-shrink-0">
                  ...
                  {avatarEl(activeChat)}
                  <div className="flex-1"><p className="text-[14px] font-medium">{activeChat.full_name}</p><p className="text-[11px] text-green-500">● Active</p></div>
                  <button onClick={() => setBlockModal(activeChat)} className="text-[11px] text-[#999] hover:text-red-500 px-3 py-1.5 border border-[#e8e8e8] rounded-full hover:border-red-200 transition-colors">Block</button>
        ...
        <div>
          {blockedUsers.length === 0
            ? <div className="text-center py-12 text-[#999]">
                ...
                <p className="text-[15px] font-medium mb-1">No blocked users</p>
                <p className="text-[13px]">Users you block will appear here.</p>
              </div>
            : <div className="flex flex-col gap-3">
                <p className="text-[13px] text-[#999] mb-1">Blocked users cannot see your profile or send you messages.</p>
                {blockedUsers.map((u: any) => (
                  <div key={u.id} className="bg-white border border-[#e8e8e8] rounded-2xl p-4 flex items-center gap-3">
                    {avatarEl(u)}
                    <div className="flex-1 min-w-0">
                      <p className="text-[14px] font-medium truncate">{u.full_name}</p>
                      <p className="text-[11px] text-[#999]">Blocked</p>
                    </div>
                    <button onClick={() => setUnblockTarget(u)} className="px-4 py-2 border border-[#e8e8e8] rounded-xl text-[13px] text-[#666] hover:bg-[#f8f7f5] transition-colors">
                      Unblock
                    </button>
                  </div>
                ))}
